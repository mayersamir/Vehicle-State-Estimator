...
void usart_Init(void)
{
	...
	CLEAR_BIT(UCSRC_Cfg,UCSRC_UCSZ0);
	CLEAR_BIT(UCSRC_Cfg,UCSRC_UCSZ1);
	CLEAR_BIT(USART_UCSRB,UCSRB_UCSZ2);
...
#if USART_INTERRUPT_CFG == ((1<<UCSRB_RXCIE))
	SET_BIT(USART_UCSRB,UCSRB_RXCIE);
	cbs.rxCallBack = NULL;
	globalInterruptManipulate(GLOBAL_INTERRUPT_ENABLE);
#elif USART_INTERRUPT_CFG == ((1<<UDRIE))
	SET_BIT(USART_UCSRB,UCSRB_UDRIE);
	cbs.udrCallBack = NULL;
	globalInterruptManipulate(GLOBAL_INTERRUPT_ENABLE);
#elif USART_INTERRUPT_CFG == ((1<<TXCIE))
	SET_BIT(USART_UCSRB,UCSRB_TXCIE);
	cbs.txCallBack = NULL;
	globalInterruptManipulate(GLOBAL_INTERRUPT_ENABLE);

#elif USART_INTERRUPT_CFG == USART_POLLING_VALUE
	/*
	 * do nothing  !
	 */
